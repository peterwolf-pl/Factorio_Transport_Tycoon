...
## Circuit rules
- Any non-zero **item signal** selects matching offers (`give.name`); the absolute value is used, so `100 iron-plate` and `-100 iron-plate` both request 100 plates. Every colony exposes the full item pool for its type, so a signal for any pool item will find a trade.
